...
export default function TechStack() {
  ...
  return (
    <section id="stack" className="section-surface relative overflow-hidden py-24 sm:py-28">
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        ...
        {/* Tab selector */}
        <div
          className="mx-auto mt-12 flex w-fit max-w-full flex-wrap justify-center gap-1.5 rounded-2xl border premium-border bg-white p-1.5 shadow-sm shadow-slate-900/5"
          role="tablist"
        ...
        >
          <div className="card-shine premium-shadow rounded-2xl border premium-border bg-white p-8 sm:p-10">
            <div className="flex flex-col gap-2 border-b border-slate-100 pb-7 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <h3 className="text-xl font-semibold tracking-tight text-slate-900">{active.title}</h3>
                <p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-600">
                  {active.description}
                </p>
              </div>
              <span className="shrink-0 rounded-full bg-yellow-50 px-3.5 py-1.5 font-mono text-xs font-bold text-slate-800">
            ...
            <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {active.items.map((item, i) => (
                <div
                  ...
                  className="group animate-fade-up rounded-xl border border-slate-200 bg-slate-50/70 px-4 py-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-teal-200 hover:bg-white hover:shadow-md hover:shadow-slate-900/8"
                >
